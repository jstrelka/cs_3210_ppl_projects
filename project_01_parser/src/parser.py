...
def errorMessage(code):
    ...
    if code == 18:
        return msg + "'=' expected"
    if code == 19:
        return msg + "identifier, if, while or '}' expected"
    if code == 20:
        return msg + "digit or ';' expected"
    if code == 21:
        return msg + "')' or ']' or '[' expected"
    if code == 22:
        return msg + "'[' or ';' expected"
    if code == 99:
...
def getErrorCode(state):
    ...
    if state in [5,80]:
        return 18
    #19 "identifier, if, or, while or '}' expected"
    if state in [57,111]:
        return 19
    # "digit or ';' expected"
    if state in [46]:
